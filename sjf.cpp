...
void sjf(vector<Process>& processes) {
    ...
    while (completed != n) {
        ...
        // Find process with shortest burst time (and earliest arrival if tie)
        for (int i = 0; i < n; i++) {
            if (!processes[i].executed && processes[i].arrival <= currentTime) {
                // If we find a shorter burst time, reset our selection
                if (processes[i].burst < minBurst) {
                    selected = i;
                    minBurst = processes[i].burst;
                    minArrival = processes[i].arrival;
                }
                // If burst time equals current minimum, pick earliest arrival
                else if (processes[i].burst == minBurst && 
                         processes[i].arrival < minArrival) {
                    selected = i;
                    minArrival = processes[i].arrival;
                }
            }
